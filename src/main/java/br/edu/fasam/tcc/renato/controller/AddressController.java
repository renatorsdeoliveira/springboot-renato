...
import br.edu.fasam.tcc.renato.service.AddressService;
...
@Log4j2
@RestController
@Api(value="Operações para manipulação dos dados de address", tags = "address, addresses")
@RequestMapping(value = "/api/address", path = "/api/address")
public class AddressController extends DefaultController implements IController<Address, Integer> {
    ...
    @ApiResponses(value={
            ...
            @ApiResponse(code = 500, message = "Erro na requisão, verifique configurações do servidor.", response = Address.class)
    })
    public ResponseEntity<?> create(@Valid @RequestBody Address entity) {

        log.trace("Criando objeto address. {}", entity);
        Address address = addressService.create(entity);
        HttpHeaders responseHttpHeaders = getHttpHeaders(address.getId());

        return ResponseEntity.status(HttpStatus.CREATED).headers(responseHttpHeaders).body(address);
    ...
    public ResponseEntity<?> read(@PathVariable Integer id) {
        ...
        return ResponseEntity.ok().headers(responseHttpHeaders).body(address);
    ...
    public ResponseEntity<?> read(@RequestParam("descricao") String descricao,
        ...
        Page<Address> list = addressService.read(descricao, PageRequest.of(page, size));
    ...
    public ResponseEntity<?> update(@PathVariable Integer id, @Valid @RequestBody Address entity) {
    ...
    public ResponseEntity<?> patch(@PathVariable Integer id, @Valid @RequestBody Address entity) {
        log.trace("Alterando registro {}", entity);
        //Atuliza o registro
        addressService.patch(entity);
        //Fazer tratativas de retorno correto HTTP
        HttpHeaders responseHeaders = getHttpHeaders(null);
        //Retornar a consulta com o cabeçalho correto
        return ResponseEntity.noContent().headers(responseHeaders).build();
